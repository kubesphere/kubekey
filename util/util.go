...
var (
	...
	ErrNoCluster = fmt.Errorf("no %q label present", kubekeyv1.ClusterLabelName)
...
func GetMachineIfExists(ctx context.Context, c client.Client, namespace, name string) (*kubekeyv1.Machine, error) {
	if c == nil {
...
func IsControlPlaneMachine(machine *kubekeyv1.Machine) bool {
	_, ok := machine.ObjectMeta.Labels[kubekeyv1.MachineControlPlaneLabelName]
	return ok
}
...
func GetClusterFromMetadata(ctx context.Context, c client.Client, obj metav1.ObjectMeta) (*kubekeyv1.Cluster, error) {
	if obj.Labels[kubekeyv1.ClusterLabelName] == "" {
		return nil, errors.WithStack(ErrNoCluster)
	}
	return GetClusterByName(ctx, c, obj.Namespace, obj.Labels[kubekeyv1.ClusterLabelName])
}

// GetOwnerCluster returns the Cluster object owning the current resource.
func GetOwnerCluster(ctx context.Context, c client.Client, obj metav1.ObjectMeta) (*kubekeyv1.Cluster, error) {
	for _, ref := range obj.OwnerReferences {
		...
		if gv.Group == kubekeyv1.GroupVersion.Group {
...
func GetClusterByName(ctx context.Context, c client.Client, namespace, name string) (*kubekeyv1.Cluster, error) {
...
func GetOwnerMachine(ctx context.Context, c client.Client, obj metav1.ObjectMeta) (*kubekeyv1.Machine, error) {
	for _, ref := range obj.OwnerReferences {
		gv, err := schema.ParseGroupVersion(ref.APIVersion)
		if err != nil {
			return nil, err
		}
		if ref.Kind == "Machine" && gv.Group == kubekeyv1.GroupVersion.Group {
...
func GetMachineByName(ctx context.Context, c client.Client, namespace, name string) (*kubekeyv1.Machine, error) {
	m := &kubekeyv1.Machine{}
...
func ClusterToObjectsMapper(c client.Client, ro client.ObjectList, scheme *runtime.Scheme) (handler.MapFunc, error) {
	...
	return func(o client.Object) []ctrl.Request {
		cluster, ok := o.(*kubekeyv1.Cluster)
		if !ok {
			return nil
		}
		...
		listOpts := []client.ListOption{
			client.MatchingLabels{
				kubekeyv1.ClusterLabelName: cluster.Name,
			},
		}
