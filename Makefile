...
generate-manifests-capkk: $(CONTROLLER_GEN) $(KUSTOMIZE) ## Generate capkk manifests e.g. CRD, RBAC etc.
	...
	cd config/capkk && $(KUSTOMIZE) edit set image capkk-controller-manager-image=$(CAPKK_CONTROLLER_IMG):$(TAG) kk-controller-manager-image=$(KK_CONTROLLER_IMG):$(TAG) kk-executor-image=$(KK_EXECUTOR_IMG):$(TAG)	
