...
import (
	...
	workflowutil "github.com/kubesphere/kubekey/util/workflow/util"
)
...
func (l *LocalCopy) Execute(runtime connector.Runtime) error {
	for _, sys := range l.Manifest.Spec.OperatingSystems {
		...
		if err := workflowutil.Mkdir(dir); err != nil {
...
func (a *ArchiveDependencies) Execute(runtime connector.Runtime) error {
	...
	if err := workflowutil.Tar(src, a.Manifest.Arg.Output, src); err != nil {
...
func (u *UnArchive) Execute(runtime connector.Runtime) error {
	if err := workflowutil.Untar(u.KubeConf.Arg.Artifact, runtime.GetWorkDir()); err != nil {
...
func (m *Md5Check) Execute(runtime connector.Runtime) error {
	...
	if exist := workflowutil.IsExist(oldFile); !exist {
		return nil
	}
	...
	newMd5 := workflowutil.LocalMd5Sum(m.KubeConf.Arg.Artifact)
...
func (c *CreateMd5File) Execute(runtime connector.Runtime) error {
	oldFile := filepath.Join(runtime.GetWorkDir(), "artifact.md5")
	newMd5 := workflowutil.LocalMd5Sum(c.KubeConf.Arg.Artifact)
