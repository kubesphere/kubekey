...
| `spec.vars.image_registry.http_port` | Integer | No | Image registry HTTP service port. Derived from `auth.registry` when `plain_http=true`, or `80` if no port is specified; empty when `plain_http=false` |
...
| `spec.vars.image_registry.auth.registry` | String | No | Image registry domain name, format `host:port/project`; the port is used to derive image registry listening ports |
