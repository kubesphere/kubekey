...
|------|-----------|--------------------------------|---------|
| Packaging node | 1 | CPU: 1 core, Memory: 1 GB, Disk: 150 GB | Must access: github.com, docker.io, quay.io |
...
> **Notes**
>
> - A single host can simultaneously assume multiple roles, e.g. both a deployment node and a private image registry node.
...
Log in to the packaging node and create a `config.yaml` file:
...
Execute the following command to download kk and Web Installer:
...
Extract KubeKey tool:

```shell
tar -zxvf tools/{arch}/kubekey-xx.tgz .
```
...
> **Note**: This Web Installer includes KubeSphere installation content. If you do not need to install KubeSphere, delete `schema/kubesphere.json`.

Open the page in your browser via `http://<bootstrap-node-ip>:8080` and follow the prompts to complete the installation.
...
> **Install private image registry along with the cluster (Option 2)**
>
...
#### 5. Configure cluster parameters
