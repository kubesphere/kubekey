...
**install nfs**：为 `nfs` 组中的节点安装 nfs 服务。  
**install image_registry**：为 `image_registry` 组中的节点安装镜像仓库。目前支持两种类型的镜像仓库：harbor、registry。  
**install etcd**：为 `etcd` 组中的节点安装 etcd。  
**install cri**：为 `k8s_cluster` 组中的节点安装 cri。目前支持两种 CRI：docker、containerd。  
**kubernetes_install**：为 `k8s_cluster` 组中的节点安装 kubernetes。  
**install helm**：为已安装好的 kubernetes 集群安装额外的 helm 应用。包含：CNI（calico、cilium、flannel、hybridnet、kubeovn、multus）
...
执行流程：
1. 复制本地脚本到远程节点的 `/etc/kubekey/scripts/post_install_{{ .inventory_hostname }}.sh`
2. 设置脚本文件权限为 0755
