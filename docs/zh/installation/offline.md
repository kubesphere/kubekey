...
执行以下命令创建安装配置文件。以下示例使用 `v1.37.0`，该版本已包含在前文 `config.yaml` 示例的离线资源列表中：

```bash
./kk create config --with-kubernetes v1.37.0 -o .
```

将 `v1.37.0` 替换为实际需要的 Kubernetes 版本。请确保替换后的版本已包含在构建离线包时使用的 `spec.download.kubernetes.kube_version` 列表中。

命令执行完毕后，将生成安装配置文件 `config-v1.37.0.yaml`。

如果选择在创建集群时同时安装镜像仓库，需要在 `config-v1.37.0.yaml` 中补充镜像仓库配置：
