...
| `image_registry.http_port` | 镜像仓库 HTTP 服务端口。当 `plain_http=true` 时，默认从 `auth.registry` 中的端口派生，未指定端口则为 `80`；当 `plain_http=false` 时为空；用户显式设置仍可覆盖。 |
| `image_registry.https_port` | 镜像仓库 HTTPS 服务端口。当 `plain_http=false` 时，默认从 `auth.registry` 中的端口派生，未指定端口则为 `443`；当 `plain_http=true` 时为空；用户显式设置仍可覆盖。 |
