...
func ModuleAssert(ctx context.Context, opts internal.ExecOptions) (string, string, error) {
	...
	aa, err := newAssertArgs(ctx, opts.Args, ha)
	if err != nil {
	...
	if aa.failMsg != "" {
		return internal.StdoutFailed, aa.failMsg, errors.New(aa.failMsg)
	}
	// condition is false and msg is not empty
	if aa.msg != "" {
		return internal.StdoutFailed, aa.msg, errors.New(aa.msg)
	}

	return internal.StdoutFailed, internal.StdoutFailed, errors.New(internal.StdoutFailed)
}
