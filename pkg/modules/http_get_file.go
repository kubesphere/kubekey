...
func ModuleHttpGetFile(ctx context.Context, options ExecOptions) (string, string, error) {
	// get host variable
	...
	errC := tmpFile.Close()
	if err != nil {
		return StdoutFailed, "failed to get http file", err
	}
	if errC != nil {
		return StdoutFailed, "failed to close tmp file", errC
	}

	err = os.Rename(tmpFilePath, destParam)
	if err != nil {
		return StdoutFailed, "failed to rename file", err
	}
...
func init() {
	// Register httpGetFile module
	utilruntime.Must(RegisterModule("http_get_file", ModuleHttpGetFile))
}
