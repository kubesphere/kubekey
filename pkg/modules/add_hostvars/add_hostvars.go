...
- Similar in spirit to set_fact, but operates on multiple hosts.
...
func ModuleAddHostvars(ctx context.Context, opts internal.ExecOptions) (string, string, error) {
	...
	args, err := newAddHostvarsArgs(ctx, opts.Args, ha)
	if err != nil {
	...
	hosts, ok := ahn.([]string)
	if !ok {
		return internal.StdoutFailed, "failed to get actual hosts from given \"hosts\"", errors.Errorf("failed to get actual hosts from given \"hosts\"")
	}
