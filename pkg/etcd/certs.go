...
func (g *GenerateCerts) Execute(runtime connector.Runtime) error {
	...
	for _, host := range runtime.GetAllHosts() {
		if host.IsRole(common.ETCD) {
			...
			files = append(files, []string{fmt.Sprintf("admin-%s.crt", host.GetName()), fmt.Sprintf("admin-%s.key", host.GetName())}...)
			certsList = append(certsList, KubekeyCertEtcdMember(host.GetName(), &altName))
			files = append(files, []string{fmt.Sprintf("member-%s.crt", host.GetName()), fmt.Sprintf("member-%s.key", host.GetName())}...)
		}
		if host.IsRole(common.Master) {
			certsList = append(certsList, KubekeyCertEtcdClient(host.GetName(), &altName))
			files = append(files, []string{fmt.Sprintf("node-%s.crt", host.GetName()), fmt.Sprintf("node-%s.key", host.GetName())}...)
		}
	}
