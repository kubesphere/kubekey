...
func DeployLocalVolume(mgr *manager.Manager) error {
	...
	_, err2 := mgr.Runner.ExecuteCmd("/usr/local/bin/kubectl apply -f /etc/kubernetes/addons/local-volume.yaml", 5, true)
	if err2 != nil {
...
func DeployRBDProvisioner(mgr *manager.Manager) error {
	...
	_, err2 := mgr.Runner.ExecuteCmd("/usr/local/bin/kubectl apply -f /etc/kubernetes/addons/rbd-provisioner.yaml -n kube-system", 5, true)
	if err2 != nil {
...
func DeployGlusterFS(mgr *manager.Manager) error {
	...
	_, err2 := mgr.Runner.ExecuteCmd("/usr/local/bin/kubectl apply -f /etc/kubernetes/addons/glusterfs.yaml -n kube-system", 5, true)
	if err2 != nil {
