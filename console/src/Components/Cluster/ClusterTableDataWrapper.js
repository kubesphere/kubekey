...
const ClusterTableDataWrapper = ({ children, clusterData }) => {
        ...
        content={
          <Menu>
            ...
            <Menu.MenuItem key="addNode">
              <Link to={`/addNode/${record.metadata.name}`}>增加节点</Link>
            </Menu.MenuItem>
            <Menu.MenuItem key="deleteNode">
              <Link to={`/deleteNode/${record.metadata.name}`}>删除节点</Link>
            </Menu.MenuItem>
          </Menu>
        }
      >
        <Button type="control" size="small">
          操作
        </Button>
      </Dropdown>
    );
  ...
  const storageColumn = (_, record) => {
    if (
      record.spec &&
      record.spec.storage &&
      record.spec.storage.openebs !== undefined
    ) {
      return <p>开启</p>;
    } else {
      return <p>关闭</p>;
  ...
  const initialColumns = [
    {
      children: [
        ...
        },
        {
          title: "节点数",
          width: "8%",
          render: (_, record) => record.spec.hosts.length,
        },
        {
          title: "Kubernetes 版本",
          dataIndex: "spec.kubernetes.version",
          width: "13%",
        },
        {
          title: "自动续费证书",
        ...
        {
          title: "容器运行时",
          dataIndex: "spec.kubernetes.containerManager",
          width: "13%",
        },
