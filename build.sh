...
if [ -n "$1" ]; then 
    if [ "$1" == "-p" ] || [ "$1" == "--proxy" ]; then
        ...
        docker run --rm -e GO111MODULE=on -e GOPROXY=https://goproxy.cn -v "$PWD":/usr/src/myapp -w /usr/src/myapp golang:1.14.7 go build -ldflags "$LDFLAGS" -v -o output/kk ./kubekey.go
    else
        echo "The option should be '-p' or '--proxy'"
    fi
else
    docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp golang:1.14.7 go build -ldflags "$LDFLAGS" -v -o output/kk ./kubekey.go
fi
