...
if [ $K3S_VERSION ]; then
   ...
   do
     ...
     qsctl cp binaries/k3s/$K3S_VERSION/$arch/k3s \
           qs://kubernetes-release/k3s/releases/download/$K3S_VERSION+k3s1/linux/$arch/k3s -c qsctl-config.yaml
   done
...
if [ $RUNC_VERSION ]; then
   ...
   do
     ...
     qsctl cp binaries/runc/$RUNC_VERSION/$arch/runc.$arch \
           qs://kubernetes-release/opencontainers/runc/releases/download/$RUNC_VERSION/runc.$arch -c qsctl-config.yaml
   done
